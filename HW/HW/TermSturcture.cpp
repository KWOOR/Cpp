...
double TermStructure::value(Date d) {
	...
	double x = values_[i] + (values_[i + 1] - values_[i])*(t - t1) / (t2 - t1); //linear interpor
	return x;
}

double YieldTermStructure::df(Date d) {
	return exp(-value(d) * timeFromRef(d));
}

double YieldTermStructure::fwd(Date d1, Date d2) {
	return (df(d1) / df(d2) - 1) / (timeFromRef(d2) - timeFromRef(d1));
}
