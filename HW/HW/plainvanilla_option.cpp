...
double PlainVanillaOption::impliedVol(double mktPrice) {
	double init = 0.1;
	double tol = 1e-6;
	double x = init;
	double e = 1;
...
double PlainVanillaOption::price_change(double ds, double dvol, double dr)
{
	s_ += ds;
	sigma_ += dvol;
	r_ += dr;
...
double PlainVanillaOption::impliedVol_bisection(double mktPrice)
{
	double tol = 1e-6;
	double min = 0;
	double max = 1;
